    ...
    <div class="row nopadding">
        ...
        <div class="col-sm-3 nopadding">
            <div class="container sig-dish-img nopadding" style="background-image:  url(<?= asset('images')?>/{{$menu1->menu_img}});">
                <a style="text-transform: uppercase; -webkit-line-clamp: 1; overflow: hidden;"href="{{ url('menu/'.$menu1->menu_type.'/'.$menu1->id) }}" id="menu-style">{{$menu1->title}}</a>
            </div>
        </div>
        <div class="col-sm-3 nopadding">
            <div class="container sig-dish-img nopadding" style="background-image: url(<?= asset('images')?>/{{$menu2->menu_img}}); ">
                <a style="text-transform: uppercase; -webkit-line-clamp: 1; overflow: hidden;" href="{{ url('menu/'.$menu1->menu_type.'/'.$menu2->id) }}" id="menu-style">{{$menu2->title}}</a>
            </div>
        </div>
        <div class="col-sm-2">
            <a  href="{{URL::route('menu.index',['menu_type' => 'signature_dish'])}}" id="seemore" style="margin-bottom: 15px"> SEE MORE </a>
        </div>
    </div>
</div>